                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="col-md-3">
                            <div class="form-group">
                                <a href="{{\Illuminate\Support\Str::finish(route('files.showfile',['dir'=>'original']),"/")}}@{{file}}?force=true"
                                   download class="btn btn-primary"><i
                                        class="fa fa-download"></i> ดาวน์โหลดไฟล์เอกสาร
                                </a>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>อัพโหลดโดย:</label>
                                <p>
                                    @{{created_by.name}}
                                </p>
                            </div>
                            <div class="form-group">
                                <label>อัพโหลดเวลา:</label>
...
$txt .= '<h5 style="margin: 0;padding: 5px 0 0 0; text-align: center; color: #0009ff;"> ระบบงาน E-office</h5>';
$txt .= '<p style="margin: 0;padding: 0 0 0 5px; font-size: 12px; border-bottom: 2px dotted #0009ff; color: #0009ff;">เลขที่รับ ' . '</p>';
$txt .= '<p style="margin: 0;padding: 0 0 0 5px; font-size: 12px; border-bottom: 2px dotted #0009ff; color: #0009ff;">วันที่ ' . (date('Y-m-d')) . '</p>';
$txt .= '<p style="margin: 0;padding: 0 0 0 5px; font-size: 12px; border-bottom: 2px dotted #0009ff; color: #0009ff;">เวลา ' . date('H.i') . ' น.</p>';
$txt .= '</div>';
$txt .= '</div>';
echo $txt;
...
@section('content')
    ...
    <section class="content-header" style="margin-bottom: 27px;">
        <h1 class="pull-left" style="font-family: 'Prompt', sans-serif">
{{--            {{ucfirst(config('settings.document_label_singular'))}}--}}
            แฟ้มเอกสาร
            <small>{{$document->name}}</small>
        </h1>
        <h1 class="pull-right" style="margin-bottom: 5px;font-family: 'Prompt', sans-serif">
            <div class="dropdown" style="display: inline-block">
                <button class="btn btn-success dropdown-toggle" type="button" data-toggle="dropdown"><i
                        class="fa fa-download"></i> ดาวน์โหลดไฟล์ Zip
                    <span class="caret"></span></button>
                <ul class="dropdown-menu">
                    <li>
                        <a href="{{route('files.downloadZip',['dir'=>'all','id'=>$document->id])}}">รวมไฟล์ทั้งหมด</a>
                    </li>
                    <li>
...
{{--                            <label>{{ucfirst(config('settings.tags_label_plural'))}}:</label>--}}
                        ...
                        <div class="form-group">
                            <label>อัปโหลดโดย:</label> {{$document->createdBy->name}}
                        </div>
                        <div class="form-group">
                            <label>เมื่อวันที่/เวลา:</label>
                            <p>{!! DateThai($document->created_at) !!} <br>
                                ({{\Carbon\Carbon::parse($document->created_at)->diffForHumans()}})
                            </p>
                        </div>
                        <div class="form-group">
                            <label>แก้ไขล่าสุดเมื่อ:</label>
                            <p>{!! DateThai($document->updated_at) !!} <br>
...
{{--                        </li>--}}
                        <li class="active"><a href="#tab_files" data-toggle="tab"
                                              aria-expanded="true">ไฟล์เอกสาร</a>
                        </li>
                        @can('verify', $document)
                            <li class=""><a href="#tab_verification" data-toggle="tab"
                                            aria-expanded="false">การอนุมัติ</a></li>
                        @endcan
                        <li class=""><a href="#tab_activity" data-toggle="tab" aria-expanded="false">กิจกรรมเอกสาร</a></li>
                        @can('user manage permission')
                            <li class=""><a href="#tab_permissions" data-toggle="tab"
                                            aria-expanded="false">ผู้รับผิดชอบ</a>
                            </li>
                        @endcan
                    </ul>
                                            ...
                                            <div class="box-header">
                                                <div class="user-block">
                                                    ...
                                                    <small class="description text-gray"><b
                                                        ...
                                                        by <b>{{$file->createdBy->name}}</b>
                                                        @php $path = "app\\files\original\\";
                                                         @endphp
                                                        <br>
                                                        <b>ขนาดไฟล์ {{number_format(filesize(storage_path($path.$file->file))/1024),2}} KB</b>
{{--                                                        <b>{{ceil(filesize(storage_path($path.$file->file))/1024/1024)}} MB</b>--}}
                                                    </small>
                                                </div>
                                                <div class="pull-right box-tools">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li><a href="javascript:void(0);"
                                                               onclick="showFileModal({{json_encode($file)}})">ดูรายละเอียด</a></li>
                                                        <li>
                            ...
                            <div class="tab-pane" id="tab_verification">
                                @if ($document->status!=config('constants.STATUS.APPROVED'))
                                    ...
                                    <textarea class="form-control" name="vcomment" id="vcomment" rows="4"
                                              placeholder="ความคิดเห็นในการอนุมัติเอกสาร (ใส่หรือไม่ใส่ก็ได้)"></textarea>
                                    </div>
                                    <div class="form-group text-center">
                                        <button class="btn btn-success" type="submit" name="action" value="approve"><i
                                                class="fa fa-check"></i> อนุมัติเอกสาร
                                        </button>
                                        <button class="btn btn-danger" type="submit" name="action" value="reject"><i
                                                class="fa fa-close"></i> ปฏิเสธเอกสาร
                                        </button>
                                    </div>
                                    {!! Form::close() !!}
                                @else
                                    <div class="form-group">
                                        <span class="label label-success">อนุมัติเอกสารแล้ว</span>
                                    </div>
                                    <div class="form-group">
                                        ผู้รับรองเอกสาร: <b>{{$document->verifiedBy->name}}</b>
                                    </div>
                                    <div class="form-gorup">
                                       เวลารับรองเอกสาร: <b>{{DateThai($document->verified_at)}}</b>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    ...
                                                    <h4 class="modal-title">การให้สิทธิ์เข้าถึงเอกสาร</h4>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-sm-12">
...
{{--                                                                    this {{config('settings.document_label_singular')}}--}}
                                    ...
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th colspan="3" style="font-size: 1.8rem;">ผู้รับผิดชอบเอกสารผ่านแท็ก
{{--                                                by {{config('settings.tags_label_plural')}}--}}
                                            </th>
                                        </tr>
                                        <tr>
...
{{--                                            <th colspan="3" style="font-size: 1.8rem;">Global Permission of {{config('settings.document_label_plural')}}</th>--}}
                                            <th colspan="3" style="font-size: 1.8rem;">สิทธิ์การเข้าถึงทั้งหมดของเอกสาร</th>
                                        </tr>
                                        <tr>
                                            <th>ผู้ใช้</th>
                                            <th>สิทธิ์เข้าถึงเอกสาร</th>
